...
def load_brand_domains(file_path: str | Path) -> Dict[str, List[BrandDomain]]:
    ...
    # Keep a tiny fallback set for well-known brands that are already
    # represented in brand_keywords.csv but were missing from the domain map.
    for brand, fallback_domain in {
        "disney": "disney.com",
        "netflix": "netflix.com",
    }.items():
        entries = domains.setdefault(brand, [])
        if not any(item.domain == fallback_domain and item.match_mode == "etld1" for item in entries):
            entries.append(BrandDomain(brand=brand, domain=fallback_domain, match_mode="etld1"))
